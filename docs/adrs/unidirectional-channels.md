...
For example, we want lighting to make first class support for interactions such
...
- Evidence of funds owed is still managed communicated in the form of cheques
  and squashes.
...
- Only the sender can add, close, or expire;
...
channels from say, a mobile, that has no reliable access to L1 data.

#### Simpler subs

Removing bidirectionality makes sub steps much more straight forward.

## Discussion, Counter and Comments

#### Mid route usecase

The consumer and merchant use case is important to CLs design goals. There is
however, no on-chain distinction in whether or no the channel is treated as a
route terminal. Thus, it may be employed more generally whenever a
unidirectional channel is required.
...
[konduit.channel](https://konduit.channel). The focus of konduit is hop from an
ada back channel on Cardano to BLN. This is implemented, and working.
...
Unidirectional channels are complementary to bidirectional ones.
